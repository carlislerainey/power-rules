...
\usepackage{setspace}
\usepackage{endnotes}
\usepackage{mathtools} % loads amsmath; provides \mathclap
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{rotating}
...
% Set up PDF metadata
\hypersetup{
pdftitle={Power Rules: Practical Advice for Computing Power},
pdfauthor={Carlisle Rainey},
pdfkeywords={statistical power, sample size, experimental design, power analysis},
pdfnewwindow=true,
colorlinks=true,
linkcolor=black,
citecolor=black,
...
Experiments continue to grow in political science \citep{Druckman2021}, and designing an experiment requires us to either select an appropriate sample size or evaluate whether a given sample size is sufficient for our goals.\footnote{
  For example, the \emph{Journal of Politics} guidelines for registered reports note that ``a detailed justification of the planned sample size is essential'' (accessed May 2, 2024, persistent link: \url{https://archive.md/g7YQ3}). 
...
Statistical power, or the chance of rejecting the null hypothesis for a given treatment effect, is a common, useful way to select or evaluate a sample size.
...
Nonetheless, \cite{ArelBundock2022} argue that only about 10\% of statistical tests in political science have at least 80\% power. 
...
One apparent problem is that while political scientists understand estimation and inference for experimental data very well (e.g., least squares regression with robust standard errors), they are less comfortable thinking about statistical power prior to data collection. 
...
The language (e.g., Cohen's \emph{d}) and style of analysis (e.g., ANCOVA) can make the software awkward for political scientists to use (though of course not insurmountable).\footnote{
...
Most importantly, political scientists tend to focus on treatment effects on the scale of the outcome (e.g., \citealt{King2000}), while psychologists are more comfortable with standardized effect sizes.\footnote{
  ...
  For example, Lakens centers much of his discussion around Cohen's \emph{d}, a standardized effect size used often in psychology but less often in political science.
  } 
...
In political science, recent conceptual work and statistical software \citep{Blair2019, Blair2023} make it more accessible to use complex simulations to comprehensively evaluate research designs, but simulation to obtain statistical power remains challenging compared to analyzing experimental data.
...
In this paper, I offer several simple rules for assessing the statistical power of a balanced, between-subjects design, including new advice on how to use pilot data to automate the power analysis.
...
There is tremendous value in simple, conceptual rules of thumb---the kind of rules that allow us to predict the statistical power of an experiment with paper, a pencil, and (perhaps) a pocket calculator. %\footnote{
...
(1) simple rules can make statistical power accessible, rather than seeming like a mystical quantity; 
...
An experiment might use more than two conditions, but for the sake of the discussion below, I focus on the statistical power of a comparison of just \emph{two} of these conditions, which I call treatment and control.\footnote{
   ...
   In this case, we need to compute the statistical power for each test separately.
...
To make inferences about $\tau$, we randomly assign $n_{1}$ respondents to treatment and $n_{0}$ respondents to control, where $N = n_{1} + n_{0}$.\footnote{
  Note that $N$ represents the \emph{total number of respondents} in the comparison, while $n_{i}$ represents the number of respondents \emph{in condition} $i$.
...
We create an indicator variable $D_{i}$ that equals 1 if the respondent is assigned to treatment and 0 if the respondent is assigned to control. 
...
In practice, we use an HC2 heteroskedasticity-robust variance estimator (or a closely related variant), which \cite{Samii2012} show is equivalent to Neyman's estimator. 
For testing, \cite{Li2017} provide several additional (seemingly innocuous) regularity conditions such that a finite-sample version of the central limit theorem holds and $\left\lbrack \widehat{\tau} - \Phi_{std}^{-1}\left( 1 - \frac{\alpha}{2} \right) \cdot {\widehat{SE}}_{\widehat{\tau}},\widehat{\tau} + \Phi_{std}^{-1}\left( 1 - \frac{\alpha}{2} \right) \cdot {\widehat{SE}}_{\widehat{\tau}} \right\rbrack$ is an asymptotically conservative $\left( 1 - \alpha \right) \cdot 100\%$ confidence interval. 
We can use the 90\% confidence interval = $\left\lbrack \widehat{\tau} - 1.64 \cdot {\widehat{SE}}_{\widehat{\tau}},\widehat{\tau} + 1.64 \cdot {\widehat{SE}}_{\widehat{\tau}} \right\rbrack$ to conduct size-0.05 tests of directional hypotheses by rejecting a null hypothesis if and only if \emph{all} values in the 90\% confidence interval are consistent with the research hypothesis \citep{Rainey2014, McCaskey2015}.
...
Similarly, we say that the experiment has ``95\% power to detect a treatment effect of \underline{~~~~~}'' if $\widehat{\tau} - 1.64 \cdot {\widehat{SE}}_{\widehat{\tau}} > 0$ in 95\% of repeated experiments. 
By convention, 80\% power is the minimal standard for ``adequate'' statistical power \citep{Cohen1988}, but others recommend higher power, such as 95\%.\footnote{
  For example, the \textit{Journal of Politics} recommends ``a power of at least 0.90, ideally 0.95, to detect anticipated/relevant effect sizes'' for registered reports (accessed May 3, 2024; persistent link: \url{https://archive.md/g7YQ3}). 
...
I denote these assumed values with a tilde. For example, I denote the assumed treatment effect as $\widetilde{\tau}$.\footnote{
...
More precisely, how can we compute statistical power?
...
Thus, the power of the study is the percentage of the sampling distribution that is larger than 1.64 standard errors. 
...
But, more importantly, it drives home a key intuition---power is determined by the key ratio $\frac{\tau}{SE}$. 
To estimate power, we must make an informed assumption about the effect of interest and a good prediction of the standard error of the estimate.
Further, when we consider changes to the experimental design and the consequences for statistical power, it can be helpful to think about the numerator or the denominator of the key ratio $\frac{\tau}{SE}$: How do we make the effect as large as
possible? And how do we make the estimate as precise as possible?
...
But obviously, we do not know the treatment effect with any precision, otherwise we would not need to run the experiment. Thus, we must assume. 
...
\emph{By default, we should focus on the statistical power for the smallest substantively meaningful effect.}\footnote{
  \cite{Rainey2014} refers to this threshold as the ``smallest substantively meaningful effect.''
  Others refer to the same concept as the ``smallest effect size of interest'' (SESOI) \citep{Lakens2018}, ``minimum effect of interest'' (MEI), and ``minimum meaningful effect'' (MME).
  }
However, when the smallest plausible effect is much larger than the smallest substantively meaningful effect, we might compute power for the smallest plausible effect.\footnote{
...
As a crude approximation, we might feel tempted to use a rule of thumb such as declaring that a 0.15 SD increase in the outcome is a ``small'' effect (e.g., \citealt{Lovakov2021}). 
...
\citet[see esp. pp. 261-263]{Lakens2018} and \citet[see esp. pp. 10-13]{Lakens2022} offer helpful further discussion of how we might choose a treatment effect of interest.
...
(Though see \citet{Perugini2014} for a conservative approach and \cite{Lakens2014} on sequential analysis; and later in this paper, I offer some new guidance for using pilot data to automate the power analysis.) 
...
After all, ``important'' is ill-defined and varies continuously. It is not at all obvious how to make this judgment. 
...
When we declare our assumption---regardless of whether we get it exactly right---we will have a useful power analysis. 
...
To motivate this rule, recall the regression model $y_{i} = \alpha + \tau D_{i} + \epsilon_{i}$, which allows us to estimate the treatment effect $\tau$ via least squares. 
...
treatment effects. 
Thus, while we should prefer robust standard errors in practice, classical standard errors allow a simple, closed-form approximation that helps us understand the power of our hypothesis tests using straightforward, helpful rules.

\section*{Rule \ref{rule:adjustment}: Adjusting for Adjustment}

We can sometimes substantially decrease the standard error by adjusting
...
For many designs, $N$ is large relative to $k$, so that $\frac{k}{(N - 3) + k} \approx 0$ and the factor becomes about $\sqrt{\left( 1 - \rho^{2} \right)}$ \citep{Bloom1995, Meyvis2017}. 
...
To find the minimum detectable effect, we need to solve for the treatment effect that positions 80\% of the sampling distribution above 1.64 standard errors. 
...
To position 80\% of the sampling distribution above 1.64 standard errors, the sampling distribution must be centered $(1.64 + 0.84) = 2.48$ standard errors above zero, which we can safely treat as 2.5. For 95\% power, we can compute $(1.64 + \Phi_{std}^{-1}(0.95)) = (1.64 + 1.64) \approx 3.3$ standard errors (see \citealt{Bloom1995} for further discussion). 
...
If we are planning to replicate the finding on a CES module, then we will have 1,000 respondents total or 500 respondents in each of the treatment and control conditions. 
...
First, Ahler and Sood ask Republicans to report their perceptions of the percent of Democrats with certain demographic attributes and ask Democrats to report their perceptions of the percent of Republicans with certain demographic attributes.
Second, Ahler and Sood provide respondents with the correct information. 
...
As part of a much larger study, \cite{Broockman2022} closely replicate Ahler and Sood's result and estimate that the treatment effect is 3.9 with a 90\% confidence interval of [1.1, 6.6].\footnote{
...
They control for a seven-point party identification scale and partisan strength. 
...
If we let $\widetilde{R^{2}} = 40\%$---though we should do additional work to confirm this working assumption\footnote{
  In a small pilot study, \cite{Cutler2024} validate a strategy of measuring the feelings toward supporters of the out-party before and after the treatment. 
...
Then we need $\widetilde{\tau} = 2.5 \cdot SE = 2.5 \cdot \frac{2 \cdot \widetilde{SD(Y)}}{\sqrt{2 \cdot n}}$.
...
If we are adjusting for control variables, we need $2 \cdot \left( \frac{2.5 \cdot \widetilde{SD(Y)} \cdot \sqrt{\left( 1 - \widetilde{R^{2}} \right)}}{\widetilde{\tau}} \right)^{2}$ respondents per condition for 80\% power. For 95\% power to detect the effect $\widetilde{\tau}$, the factor changes from 2.5 to 3.3 and we need $2 \cdot \left( \frac{3.3 \cdot \widetilde{SD(Y)}}{\widetilde{\tau}} \right)^{2}$ or $2 \cdot \left( \frac{3.3 \cdot \widetilde{SD(Y)} \cdot \sqrt{\left( 1 - \widetilde{R^2} \right)}}{\widetilde{\tau}} \right)^{2}$ respondents per condition, respectively.\footnote{
...
For 80\% power to detect the treatment effect $\widetilde{\tau}$, the sample size per condition will need to be about $2 \cdot \left( \frac{2.5 \cdot \widetilde{SD(Y)}}{\widetilde{\tau}} \right)^{2}$ (or $2 \cdot \left( \frac{2.5 \cdot \widetilde{SD(Y)} \cdot \sqrt{\left( 1 - \widetilde{R^{2}} \right)}}{\widetilde{\tau}} \right)^{2}$ if including control variables), where $\widetilde{SD(Y)}$ is the standard deviation of the outcome in a reference population and $\widetilde{R^2}$ is the $R^2$ of a regression of the outcome on the control variables in a reference population.
...
The square root law tells us that the standard error is proportional to $\frac{1}{\sqrt{n}}$. 
...
This implies a useful relationship between two studies A and B that differ in sample size.
...
For example, we should \textit{not} use this rule if (1) the original study used an unbalanced design and the planned study uses a balanced design, (2) the original study used regression adjustment and the planned study does not, (3) the original study used YouGov and the planned study uses MTurk, or (4) the original study used a seven-point Likert scale and the planned study uses a five-point Likert scale.
...
For 95\% power, we will need about $n^{existing} \cdot \left( \frac{3.3}{\widetilde{\tau}} \cdot {\widehat{SE}}_{\widehat{\tau}}^{existing} \right)^{2}$ respondents per condition.
...
Since the planned study is a direct replication of the pilot, we can perform the planned analysis on the pilot data and use Rule \ref{rule:existing-to-se} to predict the standard error (and minimum detectable
...
With a noisy estimate of the standard error, we might happen to substantially underestimate the standard error (and thus overestimate power). 
To protect against running an underpowered study, I recommend predicting the standard error \emph{conservatively} from pilot data. 

To understand how severely we might underestimate (or overestimate) the standard error using Rule \ref{rule:existing-to-se}, recall that the standard error of a sample standard deviation is about $SE\left( \text{sample SD} \right) = \text{sample SD} \cdot \sqrt{\frac{1}{4 \cdot n}}$ so that the factor $\sqrt{\frac{1}{4 \cdot n}} \cdot 100\%$ gives us a typical error in the estimate of the standard error as a percentage. 
...
To protect against an underpowered study, we can increase the predicted standard error by a factor of $2 \cdot \sqrt{\frac{1}{4 \cdot n}} + 1 = \sqrt{\frac{1}{n}} + 1$, which roughly corresponds to the upper bound of a 95\% confidence interval for the standard error. 
...
But in many contexts, we can control (or must choose) the sample size. 
...
If we mimic their design, then we can use Rule \ref{rule:existing-to-sample-size} to predict that we will need $n^{existing} \cdot \left\lbrack \frac{3.3}{\widetilde{\tau}} \cdot {\widehat{SE}}_{\widehat{\tau}}^{existing} \right\rbrack^{2} = 502 \cdot \left\lbrack \frac{3.3}{3} \cdot 1.67 \right\rbrack^{2} =1,694$ respondents per condition for 95\% power to detect a treatment effect of 3 points on the 101-point scale.\footnote{
...
\subsection*{A Pilot Study: \cite{Cutler2024}}

\cite{Cutler2024} conduct a pilot study with about 85 respondents per condition. 
...
Then we can see that $\frac{n^{B}}{n^{A}}$ = $\left( \frac{3.3}{2.5} \right)^{2}$ = 1.74. This means that to increase the power from 80\% to 95\%, we need to increase our sample size by about 74\%. 
...
Then we have potential outcomes $\mathcal{Y}_{i}(1,1)$ if assigned to both treatments, $\mathcal{Y}_{i}(1,0)$ if assigned to the first treatment, $\mathcal{Y}_{i}(0,1)$ if assigned to the second treatment, and $\mathcal{Y}_{i}(0,0)$ if assigned to neither treatment for respondent $i$ of $N$. 
...
$Y = \beta_{0} + \beta_{1} \cdot D_{1} + \beta_{2} \cdot D_{2} + \delta \cdot D_{1} \cdot D_{2} + \epsilon$.
...
The experimental design requires $n$ repetitions of each of the four unique rows of $\mathbf{X}$, which allows us to simplify $\left( \mathbf{X}^{'}\mathbf{X} \right)^{-1}$ and find that the bottom right element of $\left( \mathbf{X}^{'}\mathbf{X} \right)^{-1}\sigma^{2}$---the variance of $\widehat{\delta}$---equals $\frac{4 \cdot \sigma^2}{n}$. 
This gives us $SE_{\widehat{\delta}}^{classic} = \sqrt{\frac{4 \cdot \sigma^{2}}{n}} = \frac{2 \cdot \sigma}{\sqrt{n}}$, where $n$ represents the number of respondents assigned to each condition.\footnote{Alternatively, we can think of the estimate of the
  ...
  This is the motivation that Andrew Gelman uses in his widely-shared blog post suggesting that ``you
...
\item 
  ...
  \begin{itemize}
      ...
      If we are adjusting for control variables, we have $4 \cdot \left( \frac{2.5 \cdot \widetilde{SD(Y)} \cdot \sqrt{\left( 1 - \widetilde{R^{2}} \right)}}{\widetilde{\delta}} \right)^{2}$. 
...
They hypothesize that Republicans, for example, will report higher approval of Republican legislators who are more critical of the Democratic president, but that this effect will be much larger when the operation fails than when the operation succeeds. 
...
\cite{Myrick2020} conducted a similar experiment and measured her outcomes similarly. After analyzing her data closely, \cite{Robbins2024} expected their seven-point approval outcome would have a standard deviation of about 2.0. 
...
For 95\% power, they would need about $75 \cdot \left\lbrack \frac{3.3}{0.67} \cdot \left( \sqrt{\frac{1}{2 \cdot 75}} + 1 \right) \cdot 0.40 \right\rbrack^{2} = 340$ respondents per condition (or 1,360 respondents total) to have 95\% power.\footnote{
  ...
  Using Rule \ref{rule:pilot-to-se}, the authors would conservatively predict the standard error would be about $\sqrt{\frac{n^{pilot}}{n^{planned}}} \cdot \left( \sqrt{\frac{1}{2 \cdot n^{pilot}}} + 1 \right) \cdot {\widehat{SE}}_{\widehat{\delta}}^{pilot} = \sqrt{\frac{75}{375}} \cdot \left( \sqrt{\frac{1}{150}} + 1 \right) \cdot 0.40 = 0.19$.
...
Table \ref{tbl:how} shows how we can use the rules to establish that a planned experiment is well-powered. Broadly, there are three strategies, or ways to arrange our thinking:
...
\item For a given sample size and effect of interest, show that the statistical power of the experiment is greater than a chosen threshold (e.g., 80\% or 95\%).
...
To make each of these arguments, we can gather data from three potential sources, ideally relying on multiple sources.
...
Table \ref{tbl:mdes} provides the minimum detectable effect, in standard errors, for a variety of power levels.\footnote{I round these minimum detectable effects to the nearest tenth. For greater precision, we can use \texttt{qnorm(0.95) + qnorm(0.80)}, where \texttt{0.80} corresponds to 80\% power and can be changed to the desired power level.} 
We can use these factors rather than 2.5 for 80\% power (or 3.3 for 95\% power) in the relevant rules.
...
However, some researchers prefer to use a 95\% confidence interval, which corresponds to a size-0.025 test for directional claims. 
...
For example, these rules can help us quickly appreciate the tradeoffs between spreading survey respondents across eight conditions rather than four, or between using fewer respondents through a more expensive provider or more respondents through a less expensive provider.

In this paper, I focus mostly on the choice of sample size, but there are more (and sometimes better) ways to increase statistical power. 
Keep Rule \ref{rule:se-to-mde} top of mind: we must get the ratio of the assumed treatment effect and the standard error above 2.5 and ideally above 3.3. 
The task is clear---we need a treatment with oomph to make the numerator large and strategies like regression adjustment, precise measurement instruments, attentive respondents, and large samples to make the standard error small.\footnote{
